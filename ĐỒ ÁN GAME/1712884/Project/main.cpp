...
void HieuUng(POINT p)
{
	...
	STATE = 0;
	PlaySound(L"ambulance1.wav", NULL, SND_FILENAME);
	PlaySound(L"ambulance2.wav", NULL, SND_ASYNC);
...
void SubThread()
{
	...
	while (1)
	{
		PlaySound(L"Off Limits.wav", NULL, SND_ASYNC);
		if (STATE == true)
		{
			switch (MOVING)
			{
			...
			if (IsImpact(Y, Y.y))
			{
				PlaySound(L"smb_mariodie.wav", NULL, SND_FILENAME);
				int i = 0;
			...
			if (Y.y == 1)
			{
				...
				if (STATE == true)
				{
					PlaySound(L"smb_1-up.wav", NULL, SND_FILENAME);
					GotoXY(35, 10);
					TextColor(10);
					printf("Level up");
					TextColor(15);
...
void DrawInformationStudent(char &Choose)
{
	TextColor(1);
	GotoXY(74, 24);
...
void main()
{
		...
		if (Choose == '1')
		{
			...
			while (1)
			{
				...
				if (STATE == 1)
				{
					...
					{
						PlaySound(L"smb_pause.wav", NULL, SND_FILENAME);
						PauseGame(t1.native_handle());
						PauseGame(t2.native_handle());
					}
					else if (temp == 'L')
	...
	PlaySound(L"smb_gameover.wav", NULL, SND_FILENAME);
}